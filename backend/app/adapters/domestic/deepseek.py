...
@register_adapter(Provider.DEEPSEEK)
class DeepSeekAdapter(BaseAdapter):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.base_url = api_base or "https://api.deepseek.com"
    ...
    async def chat_completion(
        ...
        try:
            ...
            logger.error("deepseek_timeout", model=request.model, error=str(e))
            raise AdapterTimeoutError(provider=self.provider) from e

        except Exception as e:
            logger.error("deepseek_error", model=request.model, error=str(e))
            raise AdapterError(f"DeepSeek API error: {str(e)}", provider=self.provider) from e
    ...
    async def chat_completion_stream(
        ...
        try:
            ...
            logger.error("deepseek_stream_error", model=request.model, error=str(e))
            raise AdapterError(f"DeepSeek streaming error: {str(e)}", provider=self.provider) from e
    ...
    async def embedding(self, request: EmbeddingRequest) -> EmbeddingResponse:
        ...
        try:
            ...
            logger.error("deepseek_embedding_error", model=request.model, error=str(e))
            raise AdapterError(f"DeepSeek embedding error: {str(e)}", provider=self.provider) from e
...
AdapterRegistry.register_model_prefix("deepseek-", Provider.DEEPSEEK)
