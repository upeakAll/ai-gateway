"""Baichuan (百川) adapter implementation."""
...
@register_adapter(Provider.BAICHUAN)
class BaichuanAdapter(BaseAdapter):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.base_url = api_base or "https://api.baichuan-ai.com/v1"
    ...
    async def chat_completion(
        ...
        """Execute chat completion using Baichuan."""
        ...
        try:
            ...
            logger.error("baichuan_timeout", model=request.model, error=str(e))
            raise AdapterTimeoutError(provider=self.provider) from e

        except Exception as e:
            logger.error("baichuan_error", model=request.model, error=str(e))
            raise AdapterError(f"Baichuan API error: {str(e)}", provider=self.provider) from e
    ...
    async def chat_completion_stream(
        ...
        try:
            ...
            logger.error("baichuan_stream_error", model=request.model, error=str(e))
            raise AdapterError(f"Baichuan streaming error: {str(e)}", provider=self.provider) from e
    ...
    async def embedding(self, request: EmbeddingRequest) -> EmbeddingResponse:
        ...
        try:
            ...
            logger.error("baichuan_embedding_error", model=request.model, error=str(e))
            raise AdapterError(f"Baichuan embedding error: {str(e)}", provider=self.provider) from e
    ...
    def supports_model(self, model: str) -> bool:
        ...
        return model.startswith("Baichuan") or model.startswith("baichuan")

    def _parse_response(
        self, result: dict[str, Any], model: str, latency_ms: float
    ) -> ChatCompletionResponse:
        """Parse Baichuan response (OpenAI-compatible)."""
        choices = []
...
AdapterRegistry.register_model_prefix("Baichuan", Provider.BAICHUAN)
AdapterRegistry.register_model_prefix("baichuan", Provider.BAICHUAN)
